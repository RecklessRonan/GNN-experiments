...
from models import GCN, MLP, GCNConcat2, MLP_NORM
...
flags.DEFINE_string('dataset', 'ind.cora', 'Dataset string.')
flags.DEFINE_string('dataset_path', 'data', 'Path to dataset')
# 'gcn', 'gcn_cheby', 'dense'
flags.DEFINE_string('model', 'mlp_norm', 'Model string.')
...
flags.DEFINE_bool('use_signac', False,
                  'Use signac and put all args into signac workspace.')
...
flags.DEFINE_float('alpha', 0.1, 'weight for frobenius norm on Z')
...
dataset = PlanetoidData(
    FLAGS.dataset, FLAGS.dataset_path, val_size=FLAGS.val_size)
...
if FLAGS.model == 'gcn':
    ...
    if FLAGS.eigenvalue >= 0:
        support = chebyshev_polynomials(
            adj, FLAGS.max_degree, FLAGS.eigenvalue)
    else:
...
record_dict.update(test_loss=float(test_cost), test_accuracy=float(
    test_acc), test_duration=str(test_duration))

predicted_prob, predicted_class = predict(
    features, support, y_test, test_mask, placeholders)
