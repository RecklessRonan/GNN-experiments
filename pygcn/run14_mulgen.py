...
best_config = {
    ...
    'cora': [0.01, 0.6, 40, 0.00005, 0.0, 1000.0, 0.8, 2, 4, 0.9, 2, 2],
...
delta = [i/10 for i in range(8, 11)]
dropout = [0.5, 0.6, 0.7]
beta = [800, 900, 1000, 1100, 1200]
gamma = [0.7, 0.8, 0.9]

best = best_config[dataset]
for d, b, g in itertools.product(dropout, beta, gamma):
    best[1] = d
    best[5] = b
    best[6] = g
    for s in range(10):
